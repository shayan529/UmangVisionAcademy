...
const Sidebar = ({
  ...
  const NavItem = ({ item }) => (
    <NavLink
      ...
      className={({ isActive }) => {
        ...
        return `flex w-full items-center gap-3 rounded-xl border-l-2 px-3 py-2.5 text-left transition-all duration-200 ${active
            ? "border-indigo-500 bg-indigo-950/40 pl-2.5 text-indigo-300"
  ...
  const SidebarBody = () => (
    <div className="flex h-full flex-col overflow-hidden">
      <div className="shrink-0 px-3 pt-3 pb-3">
        <div className="flex items-center gap-3 rounded-xl border border-indigo-900/30 bg-indigo-950/20 p-3">
          {user?.avatarUrl ? (
            <img
              src={user.avatarUrl}
              alt={user.name}
            ...
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-400 to-violet-600 text-lg font-bold text-white">
          ...
          <div className="min-w-0 flex-1">
            ...
            {dashboardOptions.length > 1 ? (
              <div className="relative">
                {activeRoleName && (
                  <p className="mt-0.5 truncate text-[10px] font-bold uppercase tracking-wider text-indigo-400">
                    {activeRoleName}
                  </p>
                )}
                <button
                  ...
                  className="mt-1 flex cursor-pointer items-center gap-1 rounded-full border border-indigo-900/30 bg-indigo-950/40 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-indigo-400 transition-colors hover:text-indigo-300"
                >
                ...
                {rolesDropdownOpen && (
                  <div className="absolute left-0 top-full z-[120] mt-2 flex w-48 flex-col gap-1.5 rounded-xl border border-slate-800 bg-[#0f172a] p-2 shadow-2xl">
                    {dashboardOptions.map((opt, idx) => (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => { setRolesDropdownOpen(false); navigate(opt.path); }}
                        className="w-full cursor-pointer rounded-lg border border-indigo-900/20 px-2 py-1.5 text-center text-[10px] font-bold uppercase tracking-wider text-indigo-300 transition-all hover:bg-slate-900 hover:text-white"
                      >
                        {opt.name}
                      </button>
                    ))}
      ...
      <nav className="custom-scrollbar flex flex-1 flex-col gap-1 overflow-y-auto px-3 pb-2">
        {visibleGroups.map((group) => (
          <div key={group.key}>
            <button
              onClick={() => toggleGroup(group.key)}
              className="mb-0.5 flex w-full items-center justify-between px-2 py-1.5 text-[10px] font-extrabold uppercase tracking-widest text-slate-500 transition-colors hover:text-slate-300"
            >
        ...
        {mobileOpen && (
          <button
            ...
            className="mt-2 flex w-full cursor-pointer items-center gap-3 rounded-xl border-l-2 border-transparent px-3 py-2.5 text-rose-400 transition-all duration-200 hover:bg-rose-950/20 hover:text-rose-300"
          >
      ...
      {isMultiRole && mobileOpen && (
        <div className="shrink-0 border-t border-slate-800/60 px-3 py-3">
          <Link
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-600/20 transition-colors hover:bg-indigo-500"
          >
  ...
  return (
    <>
      <aside className="sticky top-0 z-40 hidden h-screen w-[260px] min-w-[260px] shrink-0 flex-col border-r border-slate-800 bg-slate-950 md:flex">
        <SidebarBody />
      </aside>
      ...
      {mobileOpen && (
        <div
          onClick={() => setMobileOpen(false)}
          className="fixed inset-x-0 top-[64px] bottom-0 z-30 bg-slate-950/75 backdrop-blur-xs md:hidden"
        />
      )}

      <aside
        className={`fixed top-[64px] bottom-0 left-0 z-40 h-[calc(100vh-64px)] w-[260px] flex-col border-r border-slate-800 bg-slate-950 shadow-[4px_0_24px_rgba(0,0,0,0.6)] transition-transform duration-300 ease-in-out md:hidden ${mobileOpen ? "flex translate-x-0" : "hidden -translate-x-full"
          }`}
      >
        <button
          onClick={() => setMobileOpen(false)}
          aria-label={t("nav.close", "Close menu")}
          className="mr-3 mt-3 mb-1 flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center self-end rounded-lg text-slate-500 transition-colors hover:bg-slate-900 hover:text-white"
        >
          <X size={17} />
        </button>
        <SidebarBody />
