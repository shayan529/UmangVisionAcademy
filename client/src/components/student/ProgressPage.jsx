...
const ProgressPage = () => {
    ...
    const completedCourses = enrolled.filter((c) => {
        ...
        if (c.completedLessons && c.totalLessons) return c.completedLessons >= c.totalLessons;
    ...
    const inProgress = enrolled.filter((c) => {
        const isComplete = c.progress === 100 || c.status === "completed" ||
            (c.completedLessons && c.totalLessons && c.completedLessons >= c.totalLessons);
        const hasStarted = (c.progress > 0) || (c.completedLessons > 0);
        return hasStarted && !isComplete;
    }).length;

    const notStarted = enrolled.filter((c) => {
        const hasStarted = (c.progress > 0) || (c.completedLessons > 0) || c.status === "completed";
        return !hasStarted;
    }).length;
        ...
        totalEnrolled > 0
            ? Math.round(
                enrolled.reduce((sum, c) => {
                    // derive progress from completedLessons if progress field is missing/zero
                    if (c.progress) return sum + c.progress;
                    if (c.completedLessons && c.totalLessons) {
                        return sum + Math.round((c.completedLessons / c.totalLessons) * 100);
                    }
                    return sum;
                }, 0) / totalEnrolled
            )
    ...
    const summaryCards = [
        { label: "Enrolled Courses", value: totalEnrolled, color: "#22d3ee", icon: "📚", bg: "rgba(34,211,238,0.08)", border: "#164e63" },
        { label: "Completed", value: completedCourses, color: "#4ade80", icon: "✅", bg: "rgba(74,222,128,0.08)", border: "#14532d" },
        { label: "In Progress", value: inProgress, color: "#a78bfa", icon: "▶️", bg: "rgba(167,139,250,0.08)", border: "#3b1f6e" },
        { label: "Not Started", value: notStarted, color: "#94a3b8", icon: "⏸️", bg: "rgba(148,163,184,0.08)", border: "#1e293b" },
        { label: "Certificates", value: certificates, color: "#facc15", icon: "🏅", bg: "rgba(250,204,21,0.08)", border: "#713f12" },
        { label: "Quizzes Taken", value: quizzesTaken, color: "#f472b6", icon: "📝", bg: "rgba(244,114,182,0.08)", border: "#831843" },
        { label: "Total Points", value: totalScore, color: "#fb923c", icon: "⭐", bg: "rgba(251,146,60,0.08)", border: "#7c2d12" },
    ];
        ...
        <div style={{ padding: "28px 32px", maxWidth: 900 }}>
            ...
            >
                ...
                <div style={{ flexShrink: 0 }}>
                    ...
                    >
                        ...
                        >
                            ...
                            <span style={{ fontSize: 9, color: "#475569", marginTop: 2 }}>overall</span>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ flex: 1 }}>
                    ...
                    <div style={{ height: 10, background: "#1e293b", borderRadius: 99, overflow: "hidden", marginBottom: 10 }}>
                        <div
                            style={{
                                height: "100%",
            ...
            >
                <h2 style={{ fontSize: 16, fontWeight: 700, color: "#f8fafc", margin: "0 0 18px" }}>
                    Course Breakdown
                </h2>
                ...
                {enrolled.length === 0 ? (
                    <div style={{ color: "#475569", fontSize: 14 }}>
                        ...
                        <NavLink to="/student-dashboard/my-courses" style={{ color: "#a78bfa" }}>
                            Browse courses →
                        </NavLink>
                    </div>
                ) : (
                            ...
                            >
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                                        ...
                                        <span style={{ fontSize: 12, color: statusColor, fontWeight: 700, flexShrink: 0 }}>
                                            {statusLabel}
                                        </span>
                                    </div>
