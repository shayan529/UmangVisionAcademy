...
const COUNSELLORS = [
  {
    ...
    availableSlots: ["Tomorrow, 4:00 PM", "Tomorrow, 6:30 PM", "Friday, 5:00 PM"],
  },
  {
    id: "c2",
    ...
    availableSlots: ["Tomorrow, 5:00 PM", "Saturday, 11:00 AM", "Saturday, 3:00 PM"],
  },
  {
    id: "c3",
    ...
    availableSlots: ["Friday, 6:00 PM", "Sunday, 10:00 AM", "Sunday, 4:00 PM"],
  },
  {
    id: "c4",
    name: "Adv. Sunita Rao",
    ...
    availableSlots: ["Saturday, 4:00 PM", "Sunday, 2:00 PM", "Monday, 6:00 PM"],
  },
];
...
export default function CareerCounselling() {
  ...
  const [sessionsHistory, setSessionsHistory] = useState([
    {
      ...
      status: "Completed",
      actionPlan: "Focus on Physics mechanics foundations and maintain daily 45-min math problem sets.",
    },
  ]);
      ...
      setSessionsHistory((prev) => [
        {
          ...
          topic,
          status: "Scheduled",
          actionPlan: "Session confirmed. Meeting link will be sent via SMS & Email 30 mins before start.",
        },
        ...prev,
      ]);
      toast.success("1-on-1 Career Counselling Session Scheduled Successfully!");
  ...
  const handleRunAssessment = () => {
    ...
    setQuizResult({
      ...
      summary: "You demonstrate strong logical deduction and spatial reasoning. A curriculum in Physics-Chemistry-Math with Computer Science will best leverage your analytical instincts.",
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-indigo-950 via-slate-900 to-purple-950 border border-indigo-500/20 p-6 md:p-10 mb-8 shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 text-xs font-bold uppercase tracking-wider mb-3">
              ...
              {t("careerCounselling.title", "Career Counselling & Mentorship Hub")}
            </div>
            <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight">
              {t("careerCounselling.title", "Personalized Career & Stream Advisory")}
            </h1>
          ...
          <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-2xl p-5 w-full md:w-72 shrink-0 shadow-lg">
            <div className="flex items-center justify-between text-xs font-semibold text-slate-400 mb-2">
              <span>{t("careerCounselling.quotaTitle", "Annual Career Advisory Quota")}</span>
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                isElite ? "bg-amber-500/20 text-amber-300 border border-amber-500/30" :
                isPremium ? "bg-rose-500/20 text-rose-300 border border-rose-500/30" :
                isBasic ? "bg-lime-500/20 text-lime-300 border border-lime-500/30" :
                "bg-slate-800 text-slate-400"
              }`}>
                {isElite ? "ELITE 👑" : isPremium ? "PREMIUM ⭐" : isBasic ? "BASIC" : "FREE"}
              </span>
            </div>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-black text-white">{remainingSessions}</span>
              <span className="text-slate-400 text-sm">/ {totalQuota} {t("careerCounselling.sessionsLeft", "sessions remaining")}</span>
            </div>
            <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden mt-3">
              <div
                className="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all duration-500"
                style={{ width: `${totalQuota > 0 ? (remainingSessions / totalQuota) * 100 : 0}%` }}
              />
            </div>
            {!hasPlan && (
              <Link
        ...
        <div className="flex items-center gap-2 mt-8 overflow-x-auto border-b border-white/10 pb-2">
          {[
            ...
            { id: "assessment", label: "Stream Aptitude Test", icon: BrainCircuit },
            { id: "roadmaps", label: "50+ Career Roadmaps", icon: TrendingUp },
            { id: "history", label: "Session History & Notes", icon: FileCheck },
          ].map((tab) => {
            const Icon = tab.icon;
                ...
                >
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-indigo-300 font-semibold">{c.experience}</p>
      ...
      {activeTab === "assessment" && (
        <div className="max-w-3xl mx-auto bg-slate-900/80 border border-slate-800 rounded-3xl p-6 md:p-10 shadow-2xl">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-white">Stream & Career Aptitude Diagnostic</h2>
            <p className="text-slate-400 text-xs md:text-sm mt-2">
              Answer 3 quick scenarios to identify which academic stream best aligns with your thinking patterns.
            </p>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-800/60 p-5 rounded-2xl border border-slate-700/60">
              ...
              <div className="space-y-2 text-xs">
                {[
                  { key: "sci", text: "Understanding how machines, software, and the universe operate" },
                  { key: "bio", text: "Learning how living organisms, medicines, and the human body heal" },
                  { key: "comm", text: "Analyzing markets, startup business models, investments & profits" },
                  { key: "arts", text: "Debating laws, writing stories, philosophy, and societal policies" },
                ].map((opt) => (
                  <label
                    key={opt.key}
                  ...
                  >
                    <input
                      ...
                      onChange={() => setQuizAnswers({ ...quizAnswers, q1: opt.key })}
            ...
            <div className="bg-slate-800/60 p-5 rounded-2xl border border-slate-700/60">
              ...
              <div className="space-y-2 text-xs">
                {[
                  { key: "tech", text: "Coding, puzzles, gaming mechanics, or DIY electronics" },
                  { key: "fin", text: "Crypto, financial news, stock trends, or business podcasts" },
                  { key: "creative", text: "Art, graphic design, debate, writing, or video editing" },
                  { key: "social", text: "Volunteering, biology documentaries, healthcare, or psychology" },
                ].map((opt) => (
                  <label
                    key={opt.key}
                    className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
                  ...
                  >
                    <input
                      ...
                      onChange={() => setQuizAnswers({ ...quizAnswers, q2: opt.key })}
                      className="accent-indigo-500"
                    />
                    {opt.text}
                  </label>
            ...
            <div className="bg-slate-800/60 p-5 rounded-2xl border border-slate-700/60">
              ...
              <div className="space-y-2 text-xs">
                {[
                  { key: "high_tech", text: "Fast-moving tech labs, AI companies, or robotics hubs" },
                  { key: "clinical", text: "Hospitals, medical research labs, or veterinary clinics" },
                  { key: "corporate", text: "Corporate headquarters, investment firms, or founding a startup" },
                  { key: "public", text: "Courtrooms, government administration, or international NGOs" },
                ].map((opt) => (
                  <label
                    key={opt.key}
                    className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
                  ...
                  >
                    <input
                      ...
                      onChange={() => setQuizAnswers({ ...quizAnswers, q3: opt.key })}
                      className="accent-indigo-500"
                    />
                    {opt.text}
                  </label>
            ...
            {quizResult && (
              <div className="mt-8 bg-gradient-to-br from-indigo-950/80 to-purple-950/80 border border-indigo-500/40 rounded-2xl p-6 shadow-2xl animate-fadeUp">
                ...
                <h3 className="text-xl font-extrabold text-white mb-1">{quizResult.primaryFit}</h3>
                <p className="text-xs text-purple-300 font-semibold mb-3">Secondary Fit: {quizResult.secondaryFit}</p>
                <p className="text-xs text-slate-300 leading-relaxed mb-4">{quizResult.summary}</p>
                <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-800 text-xs">
                  <span className="font-bold text-slate-300 block mb-1">Recommended Entrance Exams:</span>
                  <div className="flex flex-wrap gap-2">
                    {quizResult.recommendedExams.map((e) => (
                      <span key={e} className="bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded-md font-semibold">
                        {e}
                      </span>
                    ))}
                  </div>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span
                  ...
                  style={{ background: `${rm.color}20`, color: rm.color, border: `1px solid ${rm.color}40` }}
                >
                  {rm.stream}
                </span>
                <span className="text-[11px] font-semibold text-slate-400">Growth: {rm.growth}</span>
              </div>
              ...
              <div className="space-y-2 text-xs text-slate-300 bg-slate-800/40 p-4 rounded-2xl mb-4">
                <div>
                  <span className="text-slate-400 font-semibold block">Key Entrance Exams:</span>
                  <span className="text-white font-medium">{rm.exams}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-semibold block">Premier Target Institutions:</span>
                  <span className="text-white font-medium">{rm.topColleges}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-semibold block">Industry Compensation Band:</span>
                  <span className="text-emerald-400 font-bold">{rm.avgPackage}</span>
                </div>
              </div>

              <button
                onClick={() => {
                  setSelectedCounsellor(COUNSELLORS[idx % COUNSELLORS.length]);
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          {sessionsHistory.length === 0 ? (
            <div className="text-center py-16 bg-slate-900/50 rounded-3xl border border-slate-800">
              ...
              <p className="text-slate-400 text-sm">No counselling sessions booked yet.</p>
            </div>
          ) : (
              ...
              >
                <div className="space-y-1 flex-1">
                  <div className="flex items-center gap-3">
                    <h3 className="font-bold text-white text-base">{s.counsellor}</h3>
                    <span
                  ...
                  <p className="text-xs text-indigo-300 font-semibold">🕒 {s.date} • {s.topic}</p>
                  <p className="text-xs text-slate-300 bg-slate-800/60 p-3 rounded-xl mt-2 border border-slate-700/50">
                    <strong className="text-slate-200">Counsellor Action Plan: </strong>
                    {s.actionPlan}
                  </p>
                </div>
                {s.status === "Scheduled" && (
                  <button
                    onClick={() => toast.success("Opening in-app video room...")}
